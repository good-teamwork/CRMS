...
// @ts-ignore
// Avoid importing auth provider in mock mode to prevent session fetches
...
export default function App() {
  const useMock = import.meta.env.NEXT_PUBLIC_MOCK === '1';
  const [AuthProvider, setAuthProvider] = useState<React.ComponentType | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const mod = await import('@auth/create/react');
        if (mounted) {
          // @ts-ignore dynamic module shape
          setAuthProvider(() => mod.SessionProvider);
        }
      } catch (_e) {
        // If the module is unavailable, fall back to no auth provider
